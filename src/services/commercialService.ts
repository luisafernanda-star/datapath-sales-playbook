...
const ensureActiveSession = async (session: CommercialSession): Promise<CommercialSession> => {
  const expiresAt = session.expiresAt || tokenExpiry(session.accessToken);
  if (!expiresAt || expiresAt > Date.now() + 60000) return session;
  if (!session.refreshToken) throw new Error("Tu sesión venció. Cierra sesión e ingresa nuevamente antes de guardar.");
  const response = await fetch(`${url}/auth/v1/token?grant_type=refresh_token`, {
    method: "POST", headers: headers(), body: JSON.stringify({ refresh_token: session.refreshToken })
  });
  const payload = await response.json();
  if (!response.ok) throw new Error("Tu sesión venció. Cierra sesión e ingresa nuevamente.");
  const refreshed: CommercialSession = {
...
const authHeaders = async (session: CommercialSession) => headers((await ensureActiveSession(session)).accessToken);
...
export const commercialService = {
  ...
  async signIn(email: string, password: string): Promise<CommercialSession> {
    ...
    const session = { accessToken: payload.access_token as string, refreshToken: payload.refresh_token as string, expiresAt: Date.now() + Number(payload.expires_in || 3600) * 1000, email: payload.user.email as string, userId: payload.user.id as string };
  ...
  async getPrograms(session: CommercialSession): Promise<CommercialProgram[]> {
    const response = await fetch(`${url}/rest/v1/commercial_programs?select=*&order=name.asc`, { headers: await authHeaders(session) });
  ...
  async saveProgram(session: CommercialSession, program: CommercialProgram): Promise<CommercialProgram> {
    ...
    const response = await fetch(endpoint, {
      ...
      headers: { ...await authHeaders(session), Prefer: "return=representation" },
  ...
  async deleteProgram(session: CommercialSession, id: string) {
    const response = await fetch(`${url}/rest/v1/commercial_programs?id=eq.${id}`, {
      method: "DELETE", headers: await authHeaders(session)
    });
  ...
  async getFollowUps(session: CommercialSession): Promise<FollowUp[]> {
    const response = await fetch(`${url}/rest/v1/follow_ups?select=*&order=due_at.asc`, { headers: await authHeaders(session) });
  ...
  async saveFollowUp(session: CommercialSession, followUp: FollowUp): Promise<FollowUp> {
    ...
    const response = await fetch(endpoint, { method: followUp.id ? "PATCH" : "POST", headers: { ...await authHeaders(session), Prefer: "return=representation" }, body: JSON.stringify(body) });
    const data = await response.json();
  ...
  async deleteFollowUp(session: CommercialSession, id: string) {
    const response = await fetch(`${url}/rest/v1/follow_ups?id=eq.${id}`, { method: "DELETE", headers: await authHeaders(session) });
  ...
  async uploadFollowUpAttachment(session: CommercialSession, file: File): Promise<string> {
    ...
    const response = await fetch(`${url}/storage/v1/object/followup-attachments/${path}`, { method: "POST", headers: { apikey: anonKey ?? "", Authorization: `Bearer ${activeSession.accessToken}`, "Content-Type": file.type }, body: file });
  ...
  async getFollowUpAttachmentUrl(session: CommercialSession, path: string): Promise<string> {
    const response = await fetch(`${url}/storage/v1/object/sign/followup-attachments/${path}`, { method: "POST", headers: await authHeaders(session), body: JSON.stringify({ expiresIn: 31536000 }) });
    const data = await response.json();
  ...
  async getNotifications(session: CommercialSession): Promise<AppNotification[]> {
    ...
    const [notificationsResponse, readsResponse] = await Promise.all([
      fetch(`${url}/rest/v1/notifications?select=*&or=(expires_at.is.null,expires_at.gte.${encodeURIComponent(now)})&order=created_at.desc`, { headers: await authHeaders(session) }),
      fetch(`${url}/rest/v1/notification_reads?select=notification_id`, { headers: await authHeaders(session) })
    ]);
  ...
  async createNotification(session: CommercialSession, notification: { title: string; message: string; expiresAt?: string }): Promise<AppNotification> {
    const response = await fetch(`${url}/rest/v1/notifications`, {
      method: "POST", headers: { ...await authHeaders(session), Prefer: "return=representation" },
  ...
  async deleteNotification(session: CommercialSession, notificationId: string) {
    const response = await fetch(`${url}/rest/v1/notifications?id=eq.${notificationId}`, { method: "DELETE", headers: await authHeaders(session) });
  ...
  async getUserProfile(session: CommercialSession): Promise<UserProfile | null> {
    const response = await fetch(`${url}/rest/v1/user_profiles?select=*&user_id=eq.${session.userId}&limit=1`, { headers: await authHeaders(session) });
  ...
  async saveUserProfile(session: CommercialSession, profile: UserProfile): Promise<UserProfile> {
    const response = await fetch(`${url}/rest/v1/user_profiles?on_conflict=user_id`, {
      method: "POST", headers: { ...await authHeaders(session), Prefer: "resolution=merge-duplicates,return=representation" },
  ...
  async uploadProfileAvatar(session: CommercialSession, file: File): Promise<string> {
    ...
    const activeSession = await ensureActiveSession(session);
    const response = await fetch(`${url}/storage/v1/object/profile-avatars/${path}`, { method: "POST", headers: { apikey: anonKey ?? "", Authorization: `Bearer ${activeSession.accessToken}`, "Content-Type": file.type }, body: file });
    if (!response.ok) {
      const error = await response.json().catch(() => ({})) as { message?: string; error?: string };
      if (response.status === 401 || response.status === 403) throw new Error("Tu sesión venció o no tiene permiso. Cierra sesión e ingresa nuevamente.");
      if (response.status === 404) throw new Error("Falta activar el espacio de fotos en Supabase. Ejecuta nuevamente el archivo schema.sql.");
      throw new Error(error.message || error.error || "No fue posible subir la foto. Verifica que sea JPG, PNG, WEBP o GIF y que pese menos de 5 MB.");
    }
    return path;
  },
  async getProfileAvatarUrl(session: CommercialSession, path: string): Promise<string> {
    const response = await fetch(`${url}/storage/v1/object/sign/profile-avatars/${path}`, { method: "POST", headers: await authHeaders(session), body: JSON.stringify({ expiresIn: 31536000 }) });
    const data = await response.json();
