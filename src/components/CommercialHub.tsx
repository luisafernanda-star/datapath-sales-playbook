...
export function CommercialHub() {
  ...
  const [typeFilter, setTypeFilter] = useState<"Todos" | CommercialProgram["type"]>("Todos");
  ...
  const filteredPrograms = useMemo(() => programs.filter((program) =>
    program.status === "active" && (typeFilter === "Todos" || program.type === typeFilter) && [program.name, program.shortName, program.description, program.type]
  ...
  return <section className="commercial-page">
    ...
    <div className="commercial-toolbar"><input aria-label="Buscar programa" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar programa…" /><span>{filteredPrograms.length} resultados</span></div>
    <div className="commercial-filters" aria-label="Filtrar por modalidad">{(["Todos", "En vivo", "Asincrónico"] as const).map((type) => <button key={type} className={typeFilter === type ? "active" : ""} onClick={() => setTypeFilter(type)}>{type}</button>)}</div>
...
function ProgramCard({ program, copied, onCopy, canEdit, onEdit }: { program: CommercialProgram; copied: string; onCopy: (value: string, id: string) => void; canEdit: boolean; onEdit: () => void }) {
  ...
  return <article className="commercial-card"><div className="card-heading"><div><span className="program-code">{program.shortName}</span><h2>{program.name}</h2><p>{program.description}</p></div>{canEdit && <button className="icon-button" title="Editar" onClick={onEdit}><Pencil size={16} /></button>}</div><span className="type-badge">{program.type}</span>
    ...
    <CardSection title="Cupones">{regular.length ? <div className="coupon-list">{regular.map((coupon) => <CouponItem key={coupon.id} coupon={coupon} copied={copied} onCopy={onCopy} />)}</div> : <p className="muted">Aún no hay cupones configurados.</p>}</CardSection>
    {special.length > 0 && <CardSection title="Cupones especiales"><div className="coupon-list">{special.map((coupon) => <CouponItem key={coupon.id} coupon={coupon} copied={copied} onCopy={onCopy} />)}</div></CardSection>}
  </article>;
}
...
function ProgramEditor({ program, onCancel, onSave, onDelete }: { program: CommercialProgram; onCancel: () => void; onSave: (program: CommercialProgram) => void; onDelete: () => void }) {
  ...
  return <div className="modal-backdrop"><form className="editor-modal" onSubmit={(event) => { event.preventDefault(); onSave(draft); }}><div className="editor-heading"><div><p className="eyebrow">ADMINISTRACIÓN</p><h2>{program.id ? "Editar programa" : "Nuevo programa"}</h2></div><button type="button" className="icon-button" onClick={onCancel}>×</button></div>
    ...
    <EditorList title="Cupones regulares" addLabel="Agregar cupón" onAdd={() => addCoupon("regular")}>{draft.coupons.filter((item) => item.category === "regular").map((coupon) => <CouponEditor key={coupon.id} coupon={coupon} update={updateCoupon} remove={() => set("coupons", draft.coupons.filter((item) => item.id !== coupon.id))} />)}</EditorList>
    <EditorList title="Cupones especiales" addLabel="Agregar cupón especial" onAdd={() => addCoupon("special")}>{draft.coupons.filter((item) => item.category === "special").map((coupon) => <CouponEditor key={coupon.id} coupon={coupon} update={updateCoupon} remove={() => set("coupons", draft.coupons.filter((item) => item.id !== coupon.id))} />)}</EditorList>
